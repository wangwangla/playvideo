...
public class VideoPlayerAndroid implements VideoPlayer, SurfaceTexture.OnFrameAvailableListener {
    ...
    public boolean play (final FileHandle file) throws FileNotFoundException {
        ...
        try {
            if (file.type() == Files.FileType.Classpath || (file.type() == Files.FileType.Internal && !file.file().exists())) {
    ...
    public void xxx(){
        ...
        /* 获得视频宽长 */
        int imgHeight = player.getVideoHeight();
        float imgWidth = player.getVideoWidth();
        float imageRotView = 1;
        if(imgHeight>0&&imgWidth>0&&viewWidth>0&&viewHeight>0){
            float sWhView=(float)viewWidth/viewHeight;  //屏幕比例
            float sWhImg=(float)imgWidth/imgHeight;   //视频比例
            //长手机
            if(sWhImg>sWhView){
                imageRotView = sWhImg / sWhView;
                //
                float vHight = viewWidth/imgWidth*imgHeight;
                float hightbili = viewHeight / vHight;
                v3 = hightbili / 2; //居中
                float v4 = vHight / viewHeight * v3;
                v3-=v4;
            }else{
//				Matrix.orthoM(mProjectMatrix,0,-sWhView/sWhImg,sWhView/sWhImg,-1,1,1,3);
            }
        }
        transform = new Matrix4().setToOrtho2D(0, -v3, 1, imageRotView);
    }
}
